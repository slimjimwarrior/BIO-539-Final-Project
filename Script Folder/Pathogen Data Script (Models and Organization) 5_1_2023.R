...
#import state map of RHode Island

states <- map_data("state")

Rhode_Island_new <- subset(states, region == "rhode island")

burgdorferi_map <- ggplot() +
  geom_polygon(data = Rhode_Island_new, aes(x=long, y = lat, group = group), fill = "grey", alpha =0.8) +
...
mod0 <- glm( sqrt(total_Borrelia_burgdorferi) ~  Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod1 <- glm( sqrt(total_Borrelia_miyamotoi) ~  Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod2 <- glm( sqrt(total_Anaplasma_phagocytophilum) ~  Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod3 <- glm( sqrt(total_Babesia_microti) ~  Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod4 <- glm( sqrt(total_coinfection_individuals) ~ Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod5 <- glm( sqrt(total_coinfection_individuals) ~ Longitude + Latitude + total_Ixodes_Scapularis_nymphs, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
...
mod6 <- glm( sqrt(total_Borrelia_burgdorferi) ~ total_Ixodes_Scapularis_nymphs + Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod7 <- glm( sqrt(total_Borrelia_miyamotoi) ~ total_Ixodes_Scapularis_nymphs + Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod8 <- glm( sqrt(total_Anaplasma_phagocytophilum) ~ total_Ixodes_Scapularis_nymphs + Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod9 <- glm( sqrt(total_Babesia_microti) ~ total_Ixodes_Scapularis_nymphs + Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod10 <-glm( sqrt(total_coinfection_individuals) ~ total_Ixodes_Scapularis_nymphs + Longitude + Latitude, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
