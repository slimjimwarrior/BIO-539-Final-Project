...
```{r imported data sheets view, include=FALSE}
...
```{r spatial analysis finished, echo=FALSE}
Pathogen_by_site_spatial_analysis <- Pathogen_by_site_with_percentages %>%
  select(-c("percent_infected_nymphs", "percent_infection_Bb", "percent_infection_Borrelia_miyamotoi",
            "percent_infection_Borrelia_mayonii", "percent_infection_Babesia_microti","percent_infection_Ehrlichia_muris",
            "percent_infection_Anaplasma_phagocytophilum", "percent_infection_Powassan_DTV", "percent_total_coinfection_individuals",
...
#import data frame with map of Rhode Island
...
mod0 <- glm( sqrt(total_Borrelia_burgdorferi) ~  `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod1 <- glm( sqrt(total_Borrelia_miyamotoi) ~  `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod2 <- glm( sqrt(total_Anaplasma_phagocytophilum) ~  `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod3 <- glm( sqrt(total_Babesia_microti) ~  `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod4 <- glm( sqrt(total_coinfection_individuals) ~ `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod5 <- glm( sqrt(total_coinfection_individuals) ~ `Longitude (x) -` + `Latitude (y) +` + total_Ixodes_Scapularis_nymphs, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
...
mod6 <- glm( sqrt(total_Borrelia_burgdorferi) ~ total_Ixodes_Scapularis_nymphs + `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod7 <- glm( sqrt(total_Borrelia_miyamotoi) ~ total_Ixodes_Scapularis_nymphs + `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod8 <- glm( sqrt(total_Anaplasma_phagocytophilum) ~ total_Ixodes_Scapularis_nymphs + `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod9 <- glm( sqrt(total_Babesia_microti) ~ total_Ixodes_Scapularis_nymphs + `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
mod10 <-glm( sqrt(total_coinfection_individuals) ~ total_Ixodes_Scapularis_nymphs + `Longitude (x) -` + `Latitude (y) +`, Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
...
zn.lm_1 <- glm(sqrt(total_Borrelia_burgdorferi) ~ sqrt(total_Borrelia_miyamotoi) , Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))


zn.lm_2 <- glm(sqrt(total_Borrelia_burgdorferi) ~ sqrt(total_Anaplasma_phagocytophilum), Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))


zn.lm_3 <- glm( sqrt(total_Borrelia_burgdorferi) ~ sqrt(total_Babesia_microti), Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))

zn.lm_4 <- glm( sqrt(total_Borrelia_burgdorferi) ~ sqrt(total_Babesia_microti)+sqrt(total_Anaplasma_phagocytophilum)+sqrt(total_Borrelia_miyamotoi), Pathogen_by_site_spatial_analysisz_no_buffers, family = gaussian(link =identity))
...
```{r co-ccur plots new, echo=FALSE}
summary(pathogen_occurance_individuals)
summary(cooccur_pathogen_site_abundance)
summary(pathogen_by_site_presence)
```
...
```{r model summary outputs for spatial stuff, echo=FALSE}
